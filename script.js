...
let i = 0
function timer() {
    const engine = document.querySelector('.header__timer-extra');
    if (i < 50) {
        engine.innerHTML = i++
        setTimeout(() => timer(), 40)
    }
...
const photo = document.querySelectorAll('.main__product-info img');
...
for (let i = 0; i < pict.length; i++) {
    let src = photo[i].getAttribute('src');
    pict[i].addEventListener('click', () => {
